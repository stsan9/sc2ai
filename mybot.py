...
class StevenBot(sc2.BotAI):
        ...
        if len(self.units(NEXUS)) * 16 > len(self.units(PROBE)):
            if len(self.units(PROBE)) < self.MAX_WORKERS:
                for nexus in self.units(NEXUS).ready.noqueue:
                    if self.can_afford(PROBE) and len(self.units(PROBE)):
    ...
    async def expand(self):
        if self.units(NEXUS).amount < (self.iteration / self.IPM) and self.can_afford(NEXUS):
    ...
    async def build_offensive_buildings(self):
        if self.units(PYLON).ready.exists:
            ...
            elif len(self.units(GATEWAY)) < ((self.iteration / self.IPM)/2):
                if self.can_afford(GATEWAY) and not self.already_pending(GATEWAY):
                    await self.build(GATEWAY, near=pylon)

            if self.units(CYBERNETICSCORE).ready.exists:
                if len(self.units(STARGATE)) < ((self.iteration / self.IPM)/2):
